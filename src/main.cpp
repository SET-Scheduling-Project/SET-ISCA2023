...
int main(int argc, char** argv){
	...
	if(LS_tree){
		...
		cout << "LS finds no valid solution." << endl;
		return 0;
	}
	...
	auto search = [&](const char* method, bool has_S, bool has_T){
		...
		for(int i = 0; i < tries; ++i){
			...
			thr[i] = new std::thread(&SAEngine::SA_search, searchEngine[i], ref(try_sch[i]), ref(c), 2, SA_type);
		}
		for(int i = 0; i < tries; ++i){
			thr[i]->join();
			delete thr[i];
		...
		if(cur_sch){
			cout << method << "-SA: " << cur_sch.sch << endl;
			cout << "Struct:" << endl;
			cur_sch.sch->print_struct("\t");
			if(!IR_name.empty()){
				auto IR = cur_sch.sch->IR_gen();
				Json::StyledWriter swriter;
				string curIRName = IR_name + '_' + method + "-SA.json";
			...
			cout << method << " finds no valid solution." << endl;
		}
	};
	...
	auto our_search = [&](const char* method, const WholeSch& init_sch) -> WholeSch{
		if(!init_sch){
			cout << method << " has no starting point!" << endl;
			return init_sch;
		}
		...
		for(int i = 0; i < tries; ++i){
			...
			thr[i] = new std::thread(&SAEngine::SA_search, searchEngine[i], ref(try_sch[i]), ref(c), 0, 0);
		}
		for(int i = 0; i < tries; ++i){
			thr[i]->join();
			delete thr[i];
		...
		if(SA_sch){
			cout << method << ": " << SA_sch.sch << endl;
			cout << "Struct: " << endl;
			SA_sch.sch->print_struct("\t");
			if(!IR_name.empty()){
				auto IR = SA_sch.sch->IR_gen();
				Json::StyledWriter swriter;
				string curIRName = IR_name + '_' + method + ".json";
				std::ofstream IRfile(curIRName);
				IRfile << swriter.write(IR);
				IRfile.close();
			}
